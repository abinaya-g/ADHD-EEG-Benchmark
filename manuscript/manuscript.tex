...
\item \textbf{Commit}: \texttt{81c1b3b} (the commit current at the time the result files underlying this manuscript were generated and verified)
...
\item \textbf{Software environment}: TensorFlow/Keras and scikit-learn, run on a cloud GPU instance (NVIDIA Tesla T4). The exact package version numbers (TensorFlow, scikit-learn, NumPy, SciPy, Python) actually used for the run underlying Section 4 were not captured at run time; this project's dependency file pins \texttt{tensorflow==2.21.0}, but the run described in this manuscript used a cloud notebook environment's preinstalled TensorFlow build rather than this pinned version, and the two are not confirmed identical.
