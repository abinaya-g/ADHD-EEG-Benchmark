...
\shorttitle{Subject-Independent Classification of ADHD from Raw EEG: A Nested Cross-Validation Reassessment of CNN and Hybrid Classifiers}
\shortauthors{Abinaya G}

\title[mode=title]{Subject-Independent Classification of ADHD from Raw EEG: A Nested Cross-Validation Reassessment of CNN and Hybrid Classifiers}
...
\caption{CNN-and-hybrid-classifier pipeline (left) and the three independently evaluated end-to-end architectures (right). Left: the CNN's 64-unit penultimate dense layer is exposed both as the input to the plain CNN's own sigmoid output and as a fixed feature vector for the classical classifiers of Section 3.8, evaluated both on unaligned features and on CORAL-aligned features (Section 3.18); every classical classifier shares this same CNN feature extractor. Right: EEGNet, ShallowConvNet, and DeepConvNet are each trained and evaluated independently, under the identical nested cross-validation design shown in Figure 1.}
...
\caption{Nested, subject-grouped cross-validation (one repetition).}
\label{alg:1}
\begin{algorithmic}[1]
\Require subject list S (|S| = 121), epoch set X, labels y, inner folds k = 4
\State Shuffle the unique subject list in S under this repetition's seed
\State Partition S into 5 outer folds by round-robin, class-balanced assignment
\For{each outer fold f = 1 to 5}
\State S\_train <- subjects in the other 4 folds
\State S\_test <- subjects in fold f \Comment{held out entirely for this fold}
\State Split S\_train into 4 inner folds (GroupKFold on S\_train only)
\For{each inner fold i = 1 to 4}
\State Fit a fresh model on that inner fold's inner-train subjects
\State Monitor loss on that inner fold's inner-validation subjects
\State Record the training epoch at minimum inner-validation loss
\EndFor
\State budget <- median of the 4 recorded epoch counts
\State Fit one final model on all of S\_train for \texttt{budget} epochs \Comment{no validation split; no early-stopping signal available at this step}
\State Predict once on every subject in S\_test; save each prediction with its subject id
\EndFor
\Return per-subject predictions for this repetition
\end{algorithmic}
\end{algorithm}
...
All comparisons in Table 5 are made against the plain CNN at 128 Hz as the pre-specified reference model, restricted to the native 128 Hz condition. CNN+LR\_noCORAL is excluded from this family: it is numerically identical to CNN+LR (Section 3.18 confirms their metrics agree to at least six decimal places, since both are the same L2-penalized logistic regression fit on the same features), so including both would test the same null hypothesis twice and inflate the family size without adding information. The family is therefore 10 comparisons, not 11; the permutation p-values (and, for transparency, the bootstrap p-values) are Holm--Bonferroni corrected \cite{holm1979simple} within that 10-comparison family. The 128-to-512 Hz interpolation condition (Table 6) concerns a different scientific question (resolution sensitivity, not model superiority at the native resolution) and is corrected within its own, separate, 2-comparison family; because the interpolation condition was run for a single repetition only (Section 3.19), its 5 outer folds are a genuinely disjoint, non-reused subject partition, so the pseudo-replication concern above does not apply there and the original per-fold Wilcoxon signed-rank test \cite{wilcoxon1945individual} is retained for Table 6. A third, single, pre-specified comparison --- CNN+LR versus CNN+LR\_withCORAL, directly testing whether CORAL alignment changes performance (Section 4.7) --- is its own one-comparison family, for which Holm correction is a no-op and the raw permutation and bootstrap p-values are reported as-is. Consistent with standard practice, we do not treat a non-significant p-value as evidence of equivalence between two models or two conditions; we describe such results as "no statistically significant difference was detected" rather than as "no difference exists."
...
\textbf{Reconciling the model counts used throughout Section 4.} The raw, epoch-level prediction file underlying every result in this section (\texttt{all\_predictions.csv}, 34,544 rows: one row per epoch $\times$ model $\times$ preprocessing condition $\times$ repetition) contains 12 distinct model labels: CNN, EEGNet, ShallowConvNet, DeepConvNet, and six CNN-derived classical classifiers evaluated in three configurations (CNN+LR, CNN+LR\_noCORAL, CNN+LR\_withCORAL) plus five more evaluated in one configuration each (CNN+NLSVM, CNN+RF, CNN+GNB, CNN+KNN, CNN+LinearSVM). Table 3 and Table 4 report 11 rows because CNN+LR and CNN+LR\_noCORAL are merged into a single row (they are numerically identical, Section 3.18). Figures 3--6 report only 10 of these 12 models: alongside the CNN+LR/CNN+LR\_noCORAL merge, CNN+LR\_withCORAL is deliberately excluded from these four general-comparison figures and instead shown separately in Figure 7, which is specific to the CORAL analysis (Section 4.7) --- including a transductive, secondary-analysis configuration in the same figures as the primary inductive benchmark risked visually implying it belongs to the same comparison family, which Section 3.17 and Table 5 treat it as not doing. Table 4's 160 rows (20 model $\times$ preprocessing groups $\times$ 8 metrics) reflect that not every model was run at both preprocessing conditions: 8 models (CNN, CNN+GNB, CNN+KNN, CNN+LR, CNN+LinearSVM, CNN+NLSVM, CNN+RF, EEGNet) were evaluated at both 128 Hz and the 128-to-512 Hz interpolation condition, while the remaining 4 (CNN+LR\_noCORAL, CNN+LR\_withCORAL, DeepConvNet, ShallowConvNet) were evaluated at 128 Hz only (Section 3.19), giving 8$\times$2 + 4$\times$1 = 20 groups.
...
\caption{Subject-level confusion matrices for ten of the twelve evaluated models (CNN+LR\_noCORAL and CNN+LR\_withCORAL omitted per the note in Section 4.1) (n = 121 subjects; Control = typically developing, ADHD = attention-deficit/hyperactivity disorder). Counts reproduce the sensitivity and specificity values reported in Table 3 -- for example, EEGNet's 43 true-negative and 53 true-positive counts correspond to its specificity of 0.717 and sensitivity of 0.869, and CNN+GNB's 59 true-positive versus 7 true-negative counts illustrate the strong bias toward the ADHD class discussed above.}
...
Table 5 reports the pre-specified subject-level paired comparisons against the plain CNN at 128 Hz as the reference model (Section 3.17): one paired observation per subject (n = 121), with the paired permutation test as the primary inferential test and the paired bootstrap reported as an uncertainty estimate (95\% CI, effect size) rather than a second p-value, both Holm-corrected within this 10-comparison family. CNN+LR\_noCORAL is excluded as numerically identical to CNN+LR (Section 3.17).
...
\centering
\small
\caption{Table 5. Subject-level paired balanced-accuracy comparisons against CNN|128 Hz (n = 121 subjects; permutation test = primary p-value, bootstrap = uncertainty estimate, both Holm-corrected).}
\begin{tabular}{lllll}
\toprule
...
CNN vs. CNN+LR\_withCORAL & +0.188 & (0.068, 0.304) & 0.0241 & 0.0133 \\
