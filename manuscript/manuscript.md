...
Figure 5 shows the full distribution of balanced accuracy across the 25 paired outer-fold estimates (5 outer folds × 5 repetitions) underlying Table 5's comparisons, rather than only the summary point estimates.

![Figure 5. Distribution of balanced accuracy across the 25 paired outer-fold estimates (5 outer folds x 5 repetitions; Section 3.13) for each model, at the native 128 Hz condition. Boxes show the interquartile range and median (orange line); green triangles mark the mean. The wide spread for every model, including the best-performing ones (EEGNet, ShallowConvNet), illustrates why the paired, repeated-fold statistical tests in Table 5 -- rather than a single held-out point estimate -- are necessary to distinguish genuine model differences from fold-to-fold variability.](FIGURES/fig7_repeated_cv_distribution_balanced_accuracy.png)
...
- **Commit**: `81c1b3b` (the commit current at the time the result files underlying this manuscript were generated and verified)
...
- **Software environment**: TensorFlow/Keras and scikit-learn, run on a cloud GPU instance (NVIDIA Tesla T4). The exact package version numbers (TensorFlow, scikit-learn, NumPy, SciPy, Python) actually used for the run underlying Section 4 were not captured at run time; this project's dependency file pins `tensorflow==2.21.0`, but the run described in this manuscript used a cloud notebook environment's preinstalled TensorFlow build rather than this pinned version, and the two are not confirmed identical.
