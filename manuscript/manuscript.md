# Subject-Independent Classification of ADHD from Raw EEG: A Nested Cross-Validation Reassessment of CNN and Hybrid Classifiers
...
**Methods.** We used a public 19-channel EEG dataset of 121 children (61 with ADHD, 60 typically developing controls; 508 thirty-second epochs) recorded during a visual attention task. Models were evaluated with 5 outer folds × 4 inner folds × 5 repetitions of subject-grouped cross-validation, in which the inner loop alone controlled early stopping and the outer-test subjects never contributed to any training, validation, or model-selection step. Epoch-level predictions were aggregated to one prediction per child (primary aggregation: mean predicted probability) before computing accuracy, balanced accuracy, sensitivity, specificity, precision, F1, Matthews correlation coefficient (MCC), and the area under the receiver operating characteristic curve (AUC). Ninety-five percent confidence intervals were obtained by subject-level bootstrap (2000 resamples). Rather than treating repeated cross-validation folds as independent observations, pairwise model comparisons at the native resolution used a subject-level paired permutation test as the primary inferential test (one paired observation per subject, n = 121), with a paired bootstrap reported as an uncertainty estimate (95% CI, effect size) rather than a second p-value, both Holm–Bonferroni corrected within a 10-comparison family (excluding a numerically duplicate configuration). The resolution-sensitivity comparison (Section 4.8), whose 5 outer folds are not reused across repetitions, used a paired Wilcoxon signed-rank test with Holm–Bonferroni correction, and a single, direct, pre-specified comparison (CNN+LR versus its CORAL-aligned counterpart) used the same permutation/bootstrap procedure as its own one-comparison family.

**Results.** At the subject level, EEGNet achieved the highest balanced accuracy (0.793, 95% CI 0.723–0.862; AUC 0.902, 95% CI 0.844–0.949). Using the subject-level paired permutation test as primary (n = 121 subjects, Holm-corrected within a 10-comparison family; Section 3.17), EEGNet, ShallowConvNet (balanced accuracy 0.737, 95% CI 0.660–0.811), and the CNN-plus-logistic-regression classifier (balanced accuracy 0.743, 95% CI 0.662–0.819) showed a statistically significant balanced-accuracy advantage over the plain CNN (balanced accuracy 0.547, 95% CI 0.459–0.627). The remaining CNN-derived hybrid classifiers (radial-basis-function and linear support vector machines, random forest, k-nearest neighbours, Gaussian naive Bayes), and DeepConvNet, showed no statistically significant difference from the plain CNN under this test, despite several showing a higher point estimate. A single, direct, pre-specified comparison between the CNN-plus-logistic-regression classifier with and without CORAL alignment found no statistically significant difference (permutation p = 1.0, bootstrap p = 0.775; only 3 of 121 subjects received a different prediction), confirming that covariance alignment did not measurably change classification performance in this setting. A sensitivity analysis comparing the native, assumed 128 Hz representation against a 128-to-512 Hz FFT-interpolated representation found no statistically significant difference for either CNN or EEGNet (both p > 0.18; 5 paired outer folds each); we do not interpret this as evidence about the recording's true sampling rate.
...
Figure 2 summarizes the CNN-and-hybrid-classifier pipeline described in Sections 3.6–3.8 and 3.18 alongside the three independently evaluated architectures described in Sections 3.9–3.11.

![Figure 2. CNN-and-hybrid-classifier pipeline (left) and the three independently evaluated end-to-end architectures (right). Left: the CNN's 64-unit penultimate dense layer is exposed both as the input to the plain CNN's own sigmoid output and as a fixed feature vector for the classical classifiers of Section 3.8, evaluated both on unaligned features and on CORAL-aligned features (Section 3.18); every classical classifier shares this same CNN feature extractor. Right: EEGNet, ShallowConvNet, and DeepConvNet are each trained and evaluated independently, under the identical nested cross-validation design shown in Figure 1.](FIGURES/fig_architecture.png)
...
**Why the primary test is subject-level, not fold-level.** An earlier version of this analysis compared models using a Wilcoxon signed-rank test applied to 25 per-(repetition, outer-fold) balanced-accuracy values (5 repetitions × 5 outer folds). We do not use that procedure as the primary inferential test in this paper, because those 25 values are not 25 independent observations: every repetition reuses the same 121 subjects, merely reshuffled into a different fold partition, so a subject who is unusually easy or unusually hard to classify contributes correlated information to as many as 5 of the 25 values. Treating repeated-CV fold estimates as independent replicates in this way is a form of pseudo-replication that can understate the true sampling uncertainty and yield artificially small p-values; we return to this as a named limitation in Section 6.

**Primary procedure.** For each pairwise comparison, we instead construct exactly one paired observation per subject (n = 121): each model's per-subject prediction is the same mean-probability aggregation used for Table 3 and Table 4 (that subject's predicted probability pooled across all epochs and all 5 repetitions, thresholded at 0.5; Section 3.14), so every subject contributes a single paired data point regardless of how many folds or repetitions its epochs happened to fall into. The primary inferential test is a paired permutation test: independently for each subject, the two models' predictions are swapped with probability 0.5 (testing the null hypothesis that which model produced a given subject's prediction is exchangeable), repeated 20,000 times, with a two-sided p-value computed as the proportion of permuted differences at least as extreme as the one observed. We treat this as primary because it makes only the minimal assumption the comparison requires (exchangeability under the null), with no distributional or large-sample approximation. Alongside it we report a paired bootstrap as an uncertainty estimate rather than a second inferential test: the 121 subject indices are resampled with replacement 20,000 times, the same resampled indices are applied to both models' predictions to preserve the pairing, and both models' balanced accuracy is recomputed on each resample, yielding the mean and median difference, a 95% percentile confidence interval, and a bootstrap-based standardized effect size (the observed mean difference divided by the bootstrap standard deviation of that difference). A bootstrap p-value is also reported for transparency, but it is the permutation p-value, not the bootstrap p-value, that is Holm-corrected and referred to in the text as "the" p-value for a given comparison.

All comparisons in Table 5 are made against the plain CNN at 128 Hz as the pre-specified reference model, restricted to the native 128 Hz condition. CNN+LR_noCORAL is excluded from this family: it is numerically identical to CNN+LR (Section 3.18 confirms their metrics agree to at least six decimal places, since both are the same L2-penalized logistic regression fit on the same features), so including both would test the same null hypothesis twice and inflate the family size without adding information. The family is therefore 10 comparisons, not 11; the permutation p-values (and, for transparency, the bootstrap p-values) are Holm–Bonferroni corrected [38] within that 10-comparison family. The 128-to-512 Hz interpolation condition (Table 6) concerns a different scientific question (resolution sensitivity, not model superiority at the native resolution) and is corrected within its own, separate, 2-comparison family; because the interpolation condition was run for a single repetition only (Section 3.19), its 5 outer folds are a genuinely disjoint, non-reused subject partition, so the pseudo-replication concern above does not apply there and the original per-fold Wilcoxon signed-rank test [37] is retained for Table 6. A third, single, pre-specified comparison — CNN+LR versus CNN+LR_withCORAL, directly testing whether CORAL alignment changes performance (Section 4.7) — is its own one-comparison family, for which Holm correction is a no-op and the raw permutation and bootstrap p-values are reported as-is. Consistent with standard practice, we do not treat a non-significant p-value as evidence of equivalence between two models or two conditions; we describe such results as "no statistically significant difference was detected" rather than as "no difference exists."
...
**Reconciling the model counts used throughout Section 4.** The raw, epoch-level prediction file underlying every result in this section (`all_predictions.csv`, 34,544 rows: one row per epoch × model × preprocessing condition × repetition) contains 12 distinct model labels: CNN, EEGNet, ShallowConvNet, DeepConvNet, and six CNN-derived classical classifiers evaluated in three configurations (CNN+LR, CNN+LR_noCORAL, CNN+LR_withCORAL) plus five more evaluated in one configuration each (CNN+NLSVM, CNN+RF, CNN+GNB, CNN+KNN, CNN+LinearSVM). Table 3 and Table 4 report 11 rows because CNN+LR and CNN+LR_noCORAL are merged into a single row (they are numerically identical, Section 3.18). Figures 3–6 report only 10 of these 12 models: alongside the CNN+LR/CNN+LR_noCORAL merge, CNN+LR_withCORAL is deliberately excluded from these four general-comparison figures and instead shown separately in Figure 7, which is specific to the CORAL analysis (Section 4.7) — including a transductive, secondary-analysis configuration in the same figures as the primary inductive benchmark risked visually implying it belongs to the same comparison family, which Section 3.17 and Table 5 treat it as not doing. Table 4's 160 rows (20 model × preprocessing groups × 8 metrics) reflect that not every model was run at both preprocessing conditions: 8 models (CNN, CNN+GNB, CNN+KNN, CNN+LR, CNN+LinearSVM, CNN+NLSVM, CNN+RF, EEGNet) were evaluated at both 128 Hz and the 128-to-512 Hz interpolation condition, while the remaining 4 (CNN+LR_noCORAL, CNN+LR_withCORAL, DeepConvNet, ShallowConvNet) were evaluated at 128 Hz only (Section 3.19), giving 8×2 + 4×1 = 20 groups.
...
Figure 3 shows the subject-level ROC curves for ten of the twelve evaluated models, and Figure 4 shows the corresponding subject-level confusion matrices (Section 4.1 explains the model count).

![Figure 3. Subject-level receiver operating characteristic (ROC) curves for ten of the twelve evaluated models (CNN+LR_noCORAL is numerically identical to CNN+LR and omitted; CNN+LR_withCORAL is shown separately in Figure 7 alongside the rest of the CORAL analysis -- see the model-count note in Section 4.1), computed from the mean-predicted-probability aggregation described in Section 3.14 (n = 121 subjects each). Area-under-curve (AUC) values shown in the legend correspond to the AUC column of Table 3. EEGNet (AUC = 0.90) and ShallowConvNet (AUC = 0.86) dominate the other models across most of the false-positive-rate range; the plain CNN (AUC = 0.56) lies close to the chance diagonal.](FIGURES/fig5_subject_roc.png)

![Figure 4. Subject-level confusion matrices for ten of the twelve evaluated models (CNN+LR_noCORAL and CNN+LR_withCORAL omitted per the note in Section 4.1) (n = 121 subjects; Control = typically developing, ADHD = attention-deficit/hyperactivity disorder). Counts reproduce the sensitivity and specificity values reported in Table 3 -- for example, EEGNet's 43 true-negative and 53 true-positive counts correspond to its specificity of 0.717 and sensitivity of 0.869, and CNN+GNB's 59 true-positive versus 7 true-negative counts illustrate the strong bias toward the ADHD class discussed above.](FIGURES/fig6_confusion.png)
...
*Source: TABLE_4_CONFIDENCE_INTERVALS.csv (160 rows total: 20 model×preprocessing groups × 8 metrics). The full interval for every model and every metric (accuracy, balanced accuracy, sensitivity, specificity, precision, F1, MCC, AUC), at both preprocessing conditions, is available in the accompanying TABLES/ directory.*

The confidence intervals in Table 4 are wide relative to the point-estimate differences between several neighbouring models — for example, ShallowConvNet's balanced-accuracy interval (0.660–0.811) and CNN+LR's (0.662–0.819) overlap substantially — which is expected given the modest sample size (121 subjects) and is precisely the kind of uncertainty that a single point estimate, of the kind more commonly reported in the literature reviewed in Section 2, would obscure entirely.

Figure 5 shows these balanced-accuracy point estimates and 95% confidence intervals for ten of the twelve evaluated models side by side, visually confirming the overlap described above.

![Figure 5. Subject-level balanced accuracy with 95% bootstrap confidence intervals (2000 resamples) for ten of the twelve evaluated models (CNN+LR_noCORAL and CNN+LR_withCORAL omitted per the note in Section 4.1) at the native 128 Hz condition, corresponding to Table 4. EEGNet and ShallowConvNet's intervals lie clearly above DeepConvNet's and the plain CNN's, while the CNN-derived hybrid classifiers (CNN+LR, CNN+NLSVM, CNN+KNN, CNN+RF, CNN+LinearSVM) occupy an intermediate range with substantial mutual interval overlap.](FIGURES/fig4_model_comparison_balanced_accuracy.png)

## 4.6 Statistical comparison between models

Table 5 reports the pre-specified subject-level paired comparisons against the plain CNN at 128 Hz as the reference model (Section 3.17): one paired observation per subject (n = 121), with the paired permutation test as the primary inferential test and the paired bootstrap reported as an uncertainty estimate (95% CI, effect size) rather than a second p-value, both Holm-corrected within this 10-comparison family. CNN+LR_noCORAL is excluded as numerically identical to CNN+LR (Section 3.17).

**Table 5. Subject-level paired balanced-accuracy comparisons against CNN|128 Hz (n = 121 subjects; permutation test = primary p-value, bootstrap = uncertainty estimate, both Holm-corrected).**

| Comparison | Mean diff. | 95% CI of diff. (bootstrap) | p (permutation, Holm) | p (bootstrap, Holm) |
|---|---|---|---|---|
| CNN vs. EEGNet | +0.246 | (0.126, 0.362) | 0.0015 | 0.0000 |
| CNN vs. ShallowConvNet | +0.190 | (0.075, 0.303) | 0.0126 | 0.0104 |
| CNN vs. CNN+LR | +0.197 | (0.080, 0.308) | 0.0164 | 0.0099 |
| CNN vs. CNN+LR_withCORAL | +0.188 | (0.068, 0.304) | 0.0241 | 0.0133 |
| CNN vs. CNN+NLSVM | +0.130 | (0.016, 0.239) | 0.3240 | 0.1284 |
| CNN vs. CNN+KNN | +0.129 | (0.018, 0.236) | 0.3240 | 0.1284 |
| CNN vs. CNN+RF | +0.113 | (−0.001, 0.224) | 0.3876 | 0.2080 |
| CNN vs. CNN+LinearSVM | +0.096 | (−0.019, 0.209) | 0.5253 | 0.3021 |
| CNN vs. DeepConvNet | +0.042 | (−0.069, 0.150) | 0.7734 | 0.9082 |
| CNN vs. CNN+GNB | −0.005 | (−0.096, 0.083) | 0.9022 | 0.9085 |

*Source: TABLE_5_MODEL_COMPARISON_STATISTICS.csv, generated by `src/subject_level_paired_test.py` from `results_final/predictions/all_predictions.csv` (20,000 bootstrap resamples, 20,000 permutations, seed 12345), sorted by the primary (permutation) p-value.*

After Holm correction, four rows remain statistically significant by the primary permutation test: EEGNet, ShallowConvNet, CNN+LR, and CNN+LR_withCORAL. Counting distinct classifiers rather than table rows, three configurations show a confirmed advantage over the plain CNN — EEGNet, ShallowConvNet, and CNN+LR (with or without CORAL alignment; Section 4.7 tests these two configurations directly against each other) — while CNN+NLSVM, CNN+KNN, CNN+RF, CNN+LinearSVM, DeepConvNet, and CNN+GNB do **not** reach significance after correction under this subject-level test, despite several of them (KNN, NLSVM, RF) having reached nominal significance under the repeated-fold Wilcoxon procedure described in Section 3.17 — a direct illustration of why that procedure was not retained as the primary test. We report this as a genuine, and in part negative, finding: only EEGNet, ShallowConvNet, and the CNN+LR configuration show a subject-level balanced-accuracy advantage over the plain CNN that survives a statistically defensible test and multiplicity correction; the apparent advantage of the remaining hybrid classifiers over the plain CNN, while numerically present in Table 3, is not distinguishable from sampling variability at this sample size (n = 121 subjects) once fold-level pseudo-replication is removed. DeepConvNet does not differ significantly from the plain CNN under either test (permutation p = 0.773; mean difference +0.042, the smallest positive difference of any comparison in this table other than CNN+GNB's negative one).

Figure 6 shows the distribution of balanced accuracy across the 25 per-(repetition, outer-fold) estimates for each model, at the native 128 Hz condition. This distribution is reported descriptively, to illustrate fold-to-fold variability under repeated cross-validation; because the underlying 121 subjects are reused across repetitions (Section 3.17), the 25 points per model are not treated as 25 independent observations, and are not the basis of the significance tests reported in Table 5.

![Figure 6. Distribution of balanced accuracy across the 25 per-(repetition, outer-fold) estimates (5 outer folds x 5 repetitions; Section 3.13) for ten of the twelve evaluated models (CNN+LR_noCORAL and CNN+LR_withCORAL omitted per the note in Section 4.1), at the native 128 Hz condition, shown descriptively to illustrate fold-to-fold variability. Boxes show the interquartile range and median (orange line); green triangles mark the mean. Because these 25 estimates per model reuse the same 121 subjects across repetitions, they are not independent observations and are not the basis of Table 5's subject-level significance tests (Section 3.17); the wide spread shown here nonetheless illustrates why a single held-out point estimate would be an unreliable summary of any one model's performance.](FIGURES/fig7_repeated_cv_distribution_balanced_accuracy.png)

## 4.7 CORAL domain-adaptation analysis

CORAL-aligned features (CNN+LR_withCORAL, balanced accuracy 0.735, 95% CI 0.651–0.816) did not outperform the unaligned features (CNN+LR / CNN+LR_noCORAL, balanced accuracy 0.743, 95% CI 0.662–0.819); the point estimate is marginally lower with alignment applied, and the two intervals overlap almost completely. Because CORAL remains a substantive comparison in this study, we ran a dedicated, direct, subject-level paired test between these two specific configurations (CNN+LR versus CNN+LR_withCORAL; Section 3.17's single-comparison-family procedure, n = 121 subjects): the observed difference is +0.008 balanced accuracy in favor of the unaligned classifier (95% bootstrap CI −0.018 to 0.038, crossing zero), with the two classifiers producing different predictions for only 3 of 121 subjects. The primary permutation test finds no evidence against the null of exchangeability (p = 1.0); the bootstrap test agrees (p = 0.775). We therefore treat this as a statistically confirmed absence of a detectable difference, not merely an unconfirmed observation: CORAL alignment did not measurably change classification performance in this setting. Figure 7 shows the covariance-distance reduction achieved by CORAL alignment (left panel) alongside the resulting per-outer-fold accuracy distributions with and without alignment (right panel): the covariance distance between outer-train and outer-test feature distributions drops sharply and consistently after alignment, confirming the alignment step itself worked as intended, while the downstream accuracy distributions remain essentially overlapping — the mechanism did what it was designed to do without translating into a detectable classification benefit in this setting, a dissociation discussed further in Section 5.7.

![Figure 7. CORAL domain adaptation. Left: covariance distance (Frobenius norm) between outer-train and outer-test CNN feature distributions before and after CORAL alignment, pooled across all outer folds and repetitions -- alignment reduces this distance sharply and consistently. Right: per-outer-fold epoch-level accuracy of the downstream logistic-regression classifier without versus with CORAL alignment, showing near-identical distributions despite the large reduction in covariance distance on the left.](FIGURES/fig10_coral_covariance.png)
...
Figure 8 shows the full distribution underlying these comparisons for CNN (panel a) and EEGNet (panel b) separately.

![Figure 8a. Distribution of balanced accuracy across the 5 paired outer folds for the CNN model, native 128 Hz versus 128-to-512 Hz FFT-interpolated condition. Open circles mark individual fold values (n = 5 per condition, too few to resolve as a conventional box in the 128 Hz condition); the green triangle marks the mean.](FIGURES/fig8_resolution.png)

![Figure 8b. Distribution of balanced accuracy across the 5 paired outer folds for the EEGNet model, native 128 Hz versus 128-to-512 Hz FFT-interpolated condition, plotted on the same scale convention as panel a.](FIGURES/fig8_resolution_EEGNet.png)
...
- **Software/hardware**: TensorFlow/Keras and scikit-learn, run on a cloud GPU instance (NVIDIA Tesla T4); CUDA/cuDNN versions not captured. The exact package version numbers (TensorFlow, scikit-learn, NumPy, SciPy, Python) actually used for this run were not recorded at run time; this project's dependency file pins `tensorflow==2.21.0`, but the run used a cloud notebook environment's preinstalled build rather than this pinned version, and the two are not confirmed identical. This is a genuine reproducibility gap, stated here rather than papered over with an unverified version number.
- **Determinism**: CPU execution is deterministic under the stated seeds; GPU execution may show small run-to-run variation in some non-deterministic convolution kernels.

**Verification environment (this audit).** All numbers in Section 4 were independently re-derived from the raw prediction-level file `results_final/predictions/all_predictions.csv` (34,544 rows: one row per epoch × model × preprocessing-condition × repetition) using the environment below; this environment was used only to recompute metrics and run the statistical tests below from already-saved predictions, not to retrain any model.

- **Python**: 3.11.15
- **TensorFlow**: 2.21.0 (CPU build; not used for training in this verification pass)
- **Keras**: 3.15.1
- **NumPy**: 2.4.6
- **SciPy**: 1.17.1
- **scikit-learn**: 1.9.0
- **GPU / CUDA / cuDNN**: none available in this verification environment (CPU only)
- Every value in Table 2 and Table 3 was independently recomputed from `all_predictions.csv` and matched the corresponding table to within 1e-5; the subject-to-outer-fold partition was independently checked and confirmed to contain no cross-fold subject splits for any of the 12 models across all 5 repetitions (Section 4.2).
- Table 5's subject-level paired bootstrap and permutation test (20,000 resamples / 20,000 permutations, seed 12345) was run with `src/subject_level_paired_test.py`, included in the repository referenced above, reading directly from `all_predictions.csv`.
- All code, the automated 14-point leakage audit (`src/sanity_checks.py`), the raw prediction file, and the table/figure-generation scripts used to produce every number in Section 4 are included in the repository referenced above.
