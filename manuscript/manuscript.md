...
![Figure 6. CORAL domain adaptation. Left: covariance distance (Frobenius norm) between outer-train and outer-test CNN feature distributions before and after CORAL alignment, pooled across all outer folds and repetitions -- alignment reduces this distance sharply and consistently. Right: per-outer-fold epoch-level accuracy of the downstream logistic-regression classifier without versus with CORAL alignment, showing near-identical distributions despite the large reduction in covariance distance on the left.](FIGURES/fig10_coral_covariance.png)
