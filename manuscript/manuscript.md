...
Figure 3 shows the subject-level ROC curves for every model, and Figure 4 shows the corresponding subject-level confusion matrices.
...
![Figure 4. Subject-level confusion matrices for all ten models (n = 121 subjects; Control = typically developing, ADHD = attention-deficit/hyperactivity disorder). Counts reproduce the sensitivity and specificity values reported in Table 3 -- for example, EEGNet's 43 true-negative and 53 true-positive counts correspond to its specificity of 0.717 and sensitivity of 0.869, and CNN+GNB's 59 true-positive versus 7 true-negative counts illustrate the strong bias toward the ADHD class discussed above.](FIGURES/fig6_confusion.png)
...
Figure 5 shows these balanced-accuracy point estimates and 95% confidence intervals for all ten models side by side, visually confirming the overlap described above.
...
**Table 5. Subject-level paired balanced-accuracy comparisons against CNN|128 Hz (n = 121 subjects; paired bootstrap and paired permutation test, Holm-corrected).**
...
| CNN vs. CNN+LR_noCORAL | +0.197 | (0.080, 0.308) | 0.0110 | 0.0185 |
| CNN vs. ShallowConvNet | +0.190 | (0.075, 0.303) | 0.0110 | 0.0140 |
| CNN vs. CNN+LR_withCORAL | +0.188 | (0.068, 0.304) | 0.0133 | 0.0242 |
...
CORAL-aligned features (CNN+LR_withCORAL, balanced accuracy 0.735, 95% CI 0.651–0.816) did not outperform the unaligned features (CNN+LR / CNN+LR_noCORAL, balanced accuracy 0.743, 95% CI 0.662–0.819); the point estimate is marginally lower with alignment applied, and the two intervals overlap almost completely. We did not run a dedicated paired significance test between these two specific configurations (Table 5's reference model is the plain CNN, not CNN+LR_noCORAL), so this should be read as an observed, not a statistically confirmed, difference. Figure 7 shows the covariance-distance reduction achieved by CORAL alignment (left panel) alongside the resulting per-outer-fold accuracy distributions with and without alignment (right panel): the covariance distance between outer-train and outer-test feature distributions drops sharply and consistently after alignment, confirming the alignment step itself worked as intended, while the downstream accuracy distributions remain essentially overlapping — the mechanism did what it was designed to do without translating into a detectable classification benefit in this setting, a dissociation discussed further in Section 5.7.
...
**Correlation among repeated-CV estimates.** The 25 per-(repetition, outer-fold) balanced-accuracy values produced by the repeated-CV design (5 repetitions × 5 outer folds) reuse the same 121 subjects across repetitions and are therefore correlated, not independent; Section 3.17 explains why the primary model comparisons in Table 5 use a subject-level paired bootstrap and permutation test (one observation per subject, n = 121) rather than treating these 25 values as independent replicates. Figure 6 still reports the repeated-fold distribution, but descriptively, to show fold-to-fold variability, not as the basis for inference.
